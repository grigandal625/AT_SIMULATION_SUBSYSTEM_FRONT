import { Table, Select, Typography, Form, InputNumber } from "antd";
...
const ResourcesFormList = ({ fields, resources, template, modelId }) => {
    ...
    const collumns = [
        ...
        },
        {
            ...
            key: "resource",
            render: (field, _, i) => (
                <TinyFormItem {...field} name={[i, "resource_id"]}>
                    <ResourceSelect
                        resources={resources.filter(
                            (res) => res.resource_type_id === relevantResources[i]?.resource_type_id
                        )}
                        modelId={modelId}
                    />
                </TinyFormItem>
