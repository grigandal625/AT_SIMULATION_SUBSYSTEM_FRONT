...
export default ({ form, ...props }) => {
    ...
    return (
        <Form form={actualForm} {...props} layout="vertical">
            <Row gutter={10}>
                <Col flex="auto">
                    <TinyFormItem name="ticks" label="Количество тактов прогона" rules={[{ required: true, message: "Укажите количество тактов прогона" }]}>
                        <InputNumber step={1} style={{ width: "100%" }} min={1} placeholder="Укажите количество тактов прогона" />
                    </TinyFormItem>
                </Col>
                <Col flex="auto">
                    <TinyFormItem name="delay" label="Интервал между тактами (мс)" rules={[{ required: true, message: "Укажите интервал между тактами" }]}>
